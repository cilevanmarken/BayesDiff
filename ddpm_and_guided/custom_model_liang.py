    ...
    def fit(self, train_loader, override=True):
        ...
        if self.config.data.dataset == "CELEBA":
            if override:
                ...
                self.conv_out_la.H = torch.eye(self.conv_out_la.model[-1].n_params.shape[0]) * self.conv_out_la.prior_precision # Initialize Hessian to prior precision QUESTION: Should we initialize Hessian to prior precision? Prior precision on the diagonal
            ...
            for (X,t), y in train_loader:
                ...
                # loss_batch = F.mse_loss(self.conv_out_la.model(X), y)
                # self.conv_out_la.loss += loss_batch
                # self.conv_out_la.H += H_batch QUESTION: Should we add the Hessian for each batch?
                i+=1
            ...
            if override:
                self.conv_out_la._init_H()
                self.conv_out_la.H = torch.eye(self.conv_out_la.model[-1].n_params.shape[0]) * self.conv_out_la.prior_precision # Initialize Hessian to prior precision
                # self.conv_out_la.loss = 0
                self.conv_out_la.n_data = 0
