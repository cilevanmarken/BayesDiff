...
##########   get t sequence (note that t is different from timestep)  ########## 
    ...
    with torch.no_grad():
        ...
        for loop in tqdm.tqdm(range(n_rounds)):
            ...
            os.makedirs(os.path.join(exp_dir, 'variances'), exist_ok=True)
            os.makedirs(os.path.join(exp_dir, 'exp_tensors'), exist_ok=True)
            os.makedirs(os.path.join(exp_dir, 'var_tensors'), exist_ok=True)
            
            for i in range(args.sample_batch_size):
                # Save images and tensors
                tvu.save_image(x[i].cpu().float(), f"{exp_dir}/images/img_{img_id}.png")
                tvu.save_image(var_xt_next[i].cpu().float()*600, f"{exp_dir}/variances/var_{img_id}.png")
                torch.save(exp_xt_next[i], f"{exp_dir}/exp_tensors/exp_{img_id}_{args.sigma_noise}_{args.prior_precision}.pth")
                torch.save(var_xt_next[i], f"{exp_dir}/var_tensors/var_{img_id}_{args.sigma_noise}_{args.prior_precision}.pth")
                img_id += 1
                
                
            # Save the current state of the PyTorch random number generator
            torch_random_state_path = os.path.join(exp_dir, "torch_random_state.pth")
            torch.save(torch.random.get_rng_state(), torch_random_state_path)
